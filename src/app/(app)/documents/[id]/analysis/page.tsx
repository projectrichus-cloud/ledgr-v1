    ...
    async function load() {
      try {
        ...
        const { data: documentData, error: documentError } = await supabase
          .from("documents")
          .select(
            "id, company_id, file_name, confidence_score, processing_status, created_at"
          )
          .eq("id", documentId)
          .maybeSingle();
        ...
        if (!cancelled) {
          setDoc(documentData as DocumentRow);
        }
        ...
        // The document exists, but AI analysis is not ready yet.
        if (!extractionData) {
          if (!cancelled) {
            setState("processing");
          }
      ...
      <div className="w-full rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
        ...
        <div className="mb-3 flex items-center justify-center gap-2">
          ...
          <span className="text-sm font-medium text-indigo-600">
            AI Finance Team is working
          </span>
        </div>

        <h1 className="font-display text-2xl font-semibold text-slate-900">
          Your financial statement is being analyzed
        </h1>

        <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-500">
          We&apos;re extracting transactions, verifying the statement details,
          and preparing the analysis for your Chartered Accountant.
        </p>
        ...
        <div className="mt-6 rounded-xl bg-slate-50 p-4 text-left">
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-sm text-slate-400">
              Preparing results for CA review
            </span>
          </div>
        </div>
